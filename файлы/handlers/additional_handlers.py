...
async def send_my_bingo_command(message: types.Message):
    ...
    if not names:
        ...
        names = [row[0] for row in names]
        names = format_text_array(names)
        await message.reply(f"👤 Пользователь: @{message.from_user.username}\n\nТвои бинго: {names}  ", reply_markup=menu_keyboard)
...
dp.register_message_handler(send_bingo_command, commands=['bingo'], state=None)
...
def register_message_additional_handlers(dp: Dispatcher):
    dp.register_message_handler(send_bingo_command, text=['🌟Бинго', '/bingo'])
