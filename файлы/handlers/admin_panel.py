...
async def add_admin_start(message: types.Message, state: FSMContext):
    if message.text == '/stop':
        ...
        if is_user_admin(user_name):
            ...
            await message.answer('Введи @Ник_айди будущего админа:\nвводи без @\n\nЕсли ты передумал нажми /stop')
...
"""@dp.message_handler(text=['/watch_bingo',])
async def send_watch_bingo(message: types.Message):
    if message.text == '/stop':
        await message.reply("Процес остановлен")
        ...
        if is_user_admin(user_name):
                ...
                # Выполнение SQL-запроса для получения id бинго, у которых approved_bingo != 0
...
def register_handlers_admin_panel(dp: Dispatcher):
    ...
    dp.register_message_handler(process_check_bingo_action, state = AdminBingoStates.waiting_for_admin_action)
...
async def check_bingo_command(message: types.Message, state: FSMContext):
    ...
    # Send the photo
    await bot.send_photo(message.chat.id, photo=photo,caption='Процессдля добавления бинго в общий доступ\n\n<b>Одобрить или не одобрить?</b>', reply_markup=check_bingo_selection_keyboard)
