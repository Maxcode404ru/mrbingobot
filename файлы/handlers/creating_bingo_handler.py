...
async def get_number_cells(message: types.Message, state: FSMContext):
    if message.text == '/stop': # СДЕЛАТЬ ОБРАБОТЧИК КОТОРЫЙ РЕАГИРУЕТ НА ЧИСЛА КРОМЕ 3 4 5
        await state.finish()
        await message.reply("Создание бинго остановлено", reply_markup=menu_keyboard)
        return
    try:
        if 2 < int(message.text) < 6:
            async with state.proxy() as data:
                data['number_cells'] = int(message.text)
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True, selective=True)
    ...
    try:
        if message.content_type == 'photo':
            async with state.proxy() as data:
                ...
                file_path = await file.download(destination_file='photo\\background.png')
                data['background_theme_photo'] = 'photo\\background.png'
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True, selective=True)
...
async def get_color_selection(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply("Введи цвет в HEX формате или выбери кнопкой", reply_markup=markup)
        return
    ...
    try:
        ...
        if (is_hex_color(colors[0]) == True or is_const_color(colors[0]) == True) and (is_hex_color(colors[1]) == True or is_const_color(colors[1]) and len(colors) == 2):
            async with state.proxy() as data:
                ...
                await message.answer("Введите название бинго", reply_markup=types.ReplyKeyboardRemove())
                await Form.name_bingo.set()
        else:
            raise ValueError("Введи 2 цвета в HEX формате через запятую")
    except ValueError:
        await message.reply("Введи 2 цвета в HEX формате через запятую",  reply_markup=types.ReplyKeyboardRemove())
        return
...
async def get_color_selection(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply("Введи название цвета через запятую с пробелом",  reply_markup=types.ReplyKeyboardRemove())
        return
...
async def get_font(message: types.Message, state: FSMContext):
    ...
    await message.answer("Напиши слова для ячеек бинго через запятую\n\n😺 Не забудь про пробел после запятой!", reply_markup=types.ReplyKeyboardRemove())
...
async def get_text_cells(message: types.Message, state: FSMContext):
            ...
            if (message.text.count(', ') + 1) == data['number_cells']**2:
                ...
                if 'background_theme_photo' in data:
                    with open(data['background_theme_photo'], 'rb') as file:
                        # ваш код здесь
                        background_theme_photo = file.read()
                else:
    ...
    creature_new_bingo(data, background_theme_photo, data['font'])
    ...
    await bot.send_photo(message.chat.id, photo, caption='Проверь все ли так ты можешь что то изменить или добавить цвет загаловку ',reply_markup=menu_bingo_editor)
