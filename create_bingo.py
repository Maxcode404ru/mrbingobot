...
def creature_new_bingo(data, textFont): #background_theme_photo,
    ...
    # задаем шрифт и цвет текста
        font_name = ImageFont.truetype('fonts\Angeme.ttf', 50)
        font_info = ImageFont.truetype('fonts\Angeme.ttf', 40)
        font_text = ImageFont.truetype('fonts\Angeme.ttf', text_size-10)
    elif textFont == '2':
        # задаем шрифт и цвет текста
    ...
    if background_theme == 'Цвет':
        ...
        if background_theme == 'Фото':
            ...
            # Если data - объект Image, выполняем другое действие
                ...
                file_name = f'{data["background_photo_name"]}'

                # Сохраняем файл на диск
                img2 = Image.open(f'photo/{file_name}')
    ...
    draw.text((340, 150), 'Создано tg: @mrBingoBot ', font=font_info, fill=text_color_info, stroke_width=5, stroke_fill='black')
